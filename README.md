...
1. 打开[静态画廊](dist/index.html)（部署目录）或[拼图索引](handdraw-style-prompter/gallery/index.html)，浏览风格图片。
2. 记下喜欢的编号，例如 `041`。
...
给线上浏览用的白墙站点：悬停看条目信息，点击复制提示词，可按 group 筛选。复制内容是风格名称、参考作者，以及风格表里已有的「核心视觉特征」（有才附加）。201–216 这批附件只写了风格名，第三列故意留空，避免把示意图里的主体写进提示词。

- `site/`：页面源码，改界面只动这里
- `dist/`：构建产物，**部署只上传这个目录**；不要手改，构建时会整目录重写

目录职责、本地预览、数据更新后如何重新生成，见 [site/README.md](site/README.md)。

风格表或图片改过之后，至少要重新打包 `dist/`：

```bash
python3 scripts/build_dist.py
```

如果改的是 `styles_200_reorganized.md`（风格名称、分组、提示词等），先更新索引再打包：

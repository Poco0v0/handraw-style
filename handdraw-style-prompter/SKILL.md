...
This skill is self-contained. Numbered reference images live in this skill directory at `images/`, not next to the skill.
...
- If neither author-name + style-name nor author-name + style-name + traits is strong, last choice. Pass the configured reference asset through the image-generation tool's `referenced_image_paths` parameter. Assets live in this skill's numbered 200-style buckets: for example, #217 uses `images/individual/201-400/217_grid.jpg`.
...
- Resolve the style number to its configured reference asset. The normal fallback is `images/individual/{bucket}/{number}.png` inside this skill directory (for example, `048` maps to `images/individual/001-200/048.png`); a matching `{number}_grid.jpg` in the same bucket takes priority.
...
1. If this task has not already displayed the gallery, open `gallery/index.html` in this skill directory. When the Codex browser tool is available, call `mcp__codex_app__open_in_codex` with `target: { type: "browser", url: "<this-skill>/gallery/index.html" }` as a `file://` URL so the rendered gallery opens in the in-app browser.
...
Users can browse `gallery/index.html` for the numbered contact sheets. Contact-sheet PNGs and numbered singles both live under `images/` in this skill directory. The authoritative style content is `styles_200_reorganized.md` in this skill directory, or one level up when working inside the git clone; `references/styles.json` is a generated index and must be refreshed with `python scripts/build_library.py` after the Markdown changes.
